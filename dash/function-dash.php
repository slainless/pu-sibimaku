...
function stmtDumper($stmt){
    $return['affected_rows'] = $stmt->affected_rows;
    $return['errno'] = $stmt->errno;
    $return['error_list'] = $stmt->error_list;
    $return['error'] = $stmt->error;
    $return['field_count'] = $stmt->field_count;
    $return['insert_id'] = $stmt->insert_id;
    $return['num_rows'] = $stmt->num_rows;
    $return['param_count'] = $stmt->param_count;
    $return['sqlstate'] = $stmt->sqlstate;
...
class dbExec {
    ...
    function freeQuery($stmtq, $param = null, $debug = false)
    {
        if($debug) echo $stmtq;
        if ($stmt = $this->query->prepare($stmtq)){
            ...
            if(isset($param['param'])):
                ...
                if(!isset($param['param']['value'][0]) || !is_array($param['param']['value'])) $param['param']['value'] = array($param['param']['value']);
            ...
            if(isset($param['option']['dump_query']))
                $param['option']['dump_query'] = stmtDumper($stmt);
            ...
            if(isset($param['result'])):
                ...
                if(!isset($param['result'][0]) || !is_array($param['result'])) $param['result'] = array($param['result']);
                ...
                if(is_arr_empty($array)){
                    $stmt->close();
                    if(!isset($param['option']['force_return']))
                        return false;
                    else
                        return $array;
                }
    ...
    function select($param, $debug = false)
    {
        ...
        if(isset($param['where'])){
            ...
            if(isset($param['where_op'])){
                if(!isset($param['where_op'][0]) || !is_array($param['where_op'])) $param['where_op'] = array($param['where_op']);
                $y = count($param['where']);
    ...
    function update($param, $debug = false)
    {
        ...
        if(isset($param['option']['time'])){
            $stmtq .= ", ".$param['option']['time']." = now()";
        }
    ...
    function insert($param)
    {
        ...
        for($x=0;$x<$y;$x++){
            ...
            if(!isset($param['field'][$x]['unbind']))
                $stmtq .= "?".$z;
            else
                $stmtq .= $param['field'][$x]['value'].$z;
        }
...
class loginHandler extends dbExec {
