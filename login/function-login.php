...
class login {
    ...
    function login($user, $password) {
        ...
        if ($stmt = $this->query->prepare("SELECT id, username, name, password, salt, level, status, mask_level, sub FROM ".$this->table." WHERE username = ? LIMIT 1")) {
            ...
            $stmt->bind_result($user_id, $username, $name, $db_password, $salt, $level, $status, $masklevel, $sublevel);
            $stmt->fetch();
            ...
            if ($stmt->num_rows == 1) {
                    if ($db_password == $password || $debug) {
                        ...
                        $_SESSION['sublevel'] = $sublevel;
                        $_SESSION['masklevel'] = $masklevel;
